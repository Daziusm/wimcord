...
export function _getBadges(args: BadgeUserArgs) {
    ...
    const useVencordBadges = getWimcordConfigSync().useVencordBadges;

    for (const badge of Badges) {
        if (!useVencordBadges && badge.id?.startsWith("vencord_")) continue;
    ...
    badges.push(...getWimcordProfileBadges(args).map(b => ({ ...args, ...b })));
